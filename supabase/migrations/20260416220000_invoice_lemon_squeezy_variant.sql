-- Optional per-invoice Lemon Squeezy variant + extend portal RPC (runs after initial get_portal_invoice migration).

alter table public.invoices add column if not exists lemon_squeezy_variant_id text;

comment on column public.invoices.lemon_squeezy_variant_id is
  'Lemon Squeezy checkout variant id for /checkout/buy/{id}. Falls back to NEXT_PUBLIC_LEMON_SQUEEZY_VARIANT_ID when null.';
